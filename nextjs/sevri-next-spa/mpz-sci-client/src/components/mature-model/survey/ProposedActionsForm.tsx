...
import { getInitialValuesProposedActions, proposedActionsValidationSchema } from '@/utils/schemas/matureModel/proposedActionsForm';
...
function ProposedActionsForm({ onClose }: { onClose: () => void }) {
  ...
  const { createProposedActions, actualEvaluation } = useEvaluationState();
  const { user_id } = useGlobalState();

  const handleSubmitAndClose = (values: Evaluation) => {
    handleSubmit(values);
    onClose();
  };

  const handleSubmit = (values: Evaluation) => {
  ...
  return (
    <div className="p-6 bg-green shadow-md rounded-lg">
      <Formik
        initialValues={getInitialValuesProposedActions(actualEvaluation)}
