...
const housingUnitsSchema = z.object({
    num_finca: z.string()
        .min(1, 'Debe proporcionar un número de finca')
        .max(20, 'El número de finca no puede superar 20 caracteres'),
});

export default async function handler(req: NextApiRequest, res: NextApiResponse, user: any) {
    try {
        const parsedBody = housingUnitsSchema.safeParse(req.body);
        ...
        const response = await fetch(`${API_ODOO_REQUEST}/api/v1/property/constructions`, {
            method: 'POST',
            headers: {
        ...
        return res.status(200).json({
            success: true,
            construcciones: json.result.data,
        });
