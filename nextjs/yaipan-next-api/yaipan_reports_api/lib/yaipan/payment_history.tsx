...
export default async function handler(
  ...
  try {
    const parsedBody = paymentHistorySchema.safeParse(req.body);
    ...
    const response = await fetch(
      `${API_ODOO_REQUEST}/api/v1/payment/payment_history`,
      {
        method: "POST",
        headers: {
    ...
    if (!json.result?.success) {
      throw new Error(
        json.result?.message || json.result?.error || "Respuesta inválida desde Odoo"
      );
    }

    return res.status(200).json({
      success: true,
      payments: json.result.data ?? [],
    });
  } catch (error) {
