...
class UserController(Controller):
    ...
    @jwt
    @route('/api/v1/users', type='http', auth='public', csrf=False, methods=['GET', 'OPTIONS'])
    ...
    @route('/api/v1/users/<string:email>', type='http', auth='public', methods=['GET'])
    def users_by_email(self, email):
        ...
        if user:
            ...
            return Response(json.dumps(user_res),
                            content_type='application/json;charset=utf-8',
                            status=200)
        return Response(json.dumps({'error': 'There is no user.'}),
                        content_type='application/json;charset=utf-8',
                        status=404)
