...
class KmPerRouteReport(models.AbstractModel):
    _name = "report.waste_control.km_per_route_report"
    _description = "Reporte de Km por ruta"

    @api.model
    def _get_report_values(self, docids, data=None):
        wizard = self.env["waste_control.km_per_route_wizard"].browse(docids)
        wizard.ensure_one()
        year = wizard.year
        ...
        # Helper to check if is comercial
        ...
        for r_name in sorted(route_rows.keys()):
            ...
            rows.append({"name": r_name, "data": vals, "total": sum(vals)})
        ...
        return {
            "doc_ids": docids,
            "doc_model": "waste_control.km_per_route_wizard",
            "selected_year": year,
