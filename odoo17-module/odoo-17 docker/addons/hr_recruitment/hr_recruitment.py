...
class hr_job(osv.osv):
    ...
    _columns = {
        ...
        'color': fields.integer('Color Index'),
        'skill_ids': fields.many2many('hr.skill', 'hr_job_skill_rel', 'job_id', 'skill_id', 'Skills'),
        'custom_line_ids': fields.one2many('hr.job.custom.line', 'job_id', 'Custom Fields'),
    }
...
class hr_skill(osv.osv):
    ...
    _sql_constraints = [
        ('name_uniq', 'unique (name)', 'The name of the skill must be unique!')
    ]


class hr_job_custom_line(osv.osv):
    """ Custom Line for Job Position """
    _name = "hr.job.custom.line"
    _description = "Custom Line for Job Position"
    _columns = {
        'name': fields.char('Field Name', required=True, translate=True),
        'value': fields.char('Field Value', required=True, translate=True),
